...
describe('MEXAS flow safety guardrails', () => {
  ...
  test('keeps the SQL matcher atomic and deterministic under concurrent takers', () => {
    ...
    expectMarkersInOrder(source, [
      ...
      'select *',
      'into v_maker',
      'coalesce(b.is_cancelled, false) = false',
      "coalesce((b.data ->> 'isCancelled')::boolean, false) = false",
      'coalesce(b.is_filled, false) = false',
      "coalesce((b.data ->> 'isFilled')::boolean, false) = false",
      'coalesce(b.is_redemption, false) = false',
      "coalesce((b.data ->> 'isRedemption')::boolean, false) = false",
      "coalesce((b.data ->> 'mexasFundsReserved')::boolean, false) = true",
      "coalesce((b.data ->> 'mexasFundsReleased')::boolean, false) = false",
      'order by',
  ...
  test('browser smoke covers MEXAS production pages in desktop and mobile', () => {
    ...
    expect(readme).toContain('npx vercel build --prod')
    ...
    expect(source).toContain("response.headers.get('x-vercel-mitigated')")
    expect(source).toContain("getUrl('/checkout')")
    expect(source).toContain("'browser preflight Vercel Firewall'")
